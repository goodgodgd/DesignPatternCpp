// 컨테이너를 설계하는 기술
// 방법 3. Thin Template
// object 기반 컨테이너처럼 값을 읽을 때마다 캐스팅을 해야할 필요도 없고
// template 기반 컨테이너처럼 데이터 타입에 따라 코드 메모리가 막 늘어나지도 않는다.
...
// 내부구현은 void 포인터이지만 사용자는 객체를 주고 받을 수 있게 하자 (primitive type도 사용 가능)
// 구현은 상속 받고 인터페이스는 직접 구현한다 -> private 상속
template <typename T>
...
// 그냥 해본 실험: const cast를 node로 옮겨도 되네?
#if 0
struct node
{
	(void*) data;
