...
//	- static_cast: void* -> 다른 타입 또는 연관성 있는 캐스팅만 허용 (int->long, double->float) 큰 타입에서 작은 타입으로
//	- reinterpret_cast: 메모리 재해석, 거의 성공 but 주의 필요
//	- const_cast: 객체의 상수성, 휘발성 제거, 명시적 타입 관리 부담
//	- dynamic_cat: 상속에서 자식클래스의 포인터를 부모클래스의 포인터로 캐스팅 하는 것은 안전하므로 그냥 되지만 (부모 is a 자식)
//				반대로 부모클래스 포인터를 자식클래스 포인터로 캐스팅할 때는 dynamic_cast 필요
