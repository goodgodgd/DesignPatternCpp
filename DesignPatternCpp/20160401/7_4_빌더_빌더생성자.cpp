...
public:
	...
	{
	public:
		Builder(string i, string a) : id_(i), address_(a) {}	// 필수인자는 반드시 포함!

		Builder& level(int a)
		{
			level_ = a;
			return *this;		// 자신의 레퍼런스를 리턴하는 것이 핵심! -> 그래야 다른 setter를 연결할수 있다.
		}
		...
		// UserInfo 생성자에서 Builder의 멤버에 접근 가능하게 만든다.
...
int main()
{
	UserInfo ui = UserInfo::Builder("hyukdoo.choi", "익산")		// 필수 인자(불변 인자) 입력
									.level(10)					// 나머진 선택적 입력
									.gold(100)
									.cash(2000)
									.build();					// 반드시 build 함수로 끝나야!!
}
