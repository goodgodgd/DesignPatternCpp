...
// - 방문자 패턴은 Composite 패턴과 연계되어서 많이 사용됩니다. (Composite에 기능을 추가할 때)
...
// 모든 메뉴의 공통의 부모
// IMenuAcceptor 를 popupmenu에서 다중상속보다 Base에서 미리 받는 것이 좋다.
...
class PopupMenu : public BaseMenu
{
	...
	vector<BaseMenu*> v;	// 재귀적 합성: 자기 자신을 포함할 수 있는 설계 - Composite
	// 자식의 공통 부분은 부모로 보낸다.
	//	string title;
public:
	...
	virtual void accept(IMenuVisitor* visitor)
	{
		...
		for (BaseMenu* e : v)	// Range-for
			e->accept(visitor);
		// e가 PopupMenu이면 이 함수가 재귀적으로 실행되어 모든 하부의 하부 메뉴들까지 전달되어
		// 모든 PopupMenu와 MenuItem에서 visitor->visit(this); 가 실행된다. 
		// visitor의 연산은 this의 타입에 따라 달라질 수 있다.
	}
...
public:
	...
	virtual void visit(PopupMenu* p)
	{
		...
		p->setTitle(s);		// 캡슐화 전략 위배
							// 기존에 설계했던 BaseItem에는 setTitle() 이 없었는데 추가해야 한다.
		// 어? setTitle이 없네? 방문자의 문제는 캡슐화 전략의 위배, 기존 시스템의 설계에서는 메뉴 이름을 바꿀 수 있는 setTitle이 필요없었다.
	}
};
